...
static inline char *mp_encode_str(char *data, const char *str_val, uint32_t len)
{
	if (len <= 31) {
	...
	if (str_val && len > 0) {
		memcpy(data, str_val, (size_t)len);
		data += len;
	}
	return data;
}
...
static inline bool mp_is_str_or_bin(char c)
{
	uint8_t u = (uint8_t)c;
	return ((u & 0xe0) == MP_FIXSTR) ||
	       u == MP_STR8 || u == MP_STR16 || u == MP_STR32 ||
	       u == MP_BIN8 || u == MP_BIN16 || u == MP_BIN32;
}

static inline const char *mp_decode_bin(const char **data, uint32_t *len)
{
	const uint8_t *p = (const uint8_t *)*data;
	uint8_t c = *p++;
	uint32_t l = 0;
...
static inline const char *mp_decode_str_or_bin(const char **data, uint32_t *len)
{
	uint8_t c = (uint8_t)**data;
	if (c == MP_BIN8 || c == MP_BIN16 || c == MP_BIN32)
		return mp_decode_bin(data, len);
	return mp_decode_str(data, len);
}
